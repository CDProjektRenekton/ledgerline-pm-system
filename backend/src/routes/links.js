...
router.post("/", async (req, res) => {
  ...
  await logHistory(taskId, req.user.id, "link_added", `${req.user.name} added link "${label.trim()}"`);
...
router.delete("/:id", async (req, res) => {
  ...
  await logHistory(link.task_id, req.user.id, "link_removed", `${req.user.name} removed link "${link.label}"`);
  res.json({ ok: true });
});
